...
def predict(user_id, predictions):
    ...
    cluster = user_cluster_df.loc[user_cluster_df['UserID'] == user_id].assigned_cluster.values[0]
    user_cluster_df= user_cluster_df.loc[user_cluster_df['assigned_cluster'] == cluster]

    cluster_users_df = progress_df[progress_df['UserID'].isin(user_cluster_df['UserID'])]
    #convert progress percentage string to numeric data
